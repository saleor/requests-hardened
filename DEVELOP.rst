...
This section describes the multiple ways to set-up a development environment for this
library.

Using Docker/Containers
-----------------------

The fastest way, is to create a development container.

Using the ``make`` Command
^^^^^^^^^^^^^^^^^^^^^^^^^^

If you have ``make`` installed (supported in Linux, MacOS, or using Cygwin on Windows),
then you can run ``make shell``, this will build a container image and launch a shell.
...
1. Build the image:
   ...
   .. code-block:: bash
     ...
     $ docker build --progress=plain -t "requests-hardened" .
...
Using the Machine's Python Interpret (Local Way)
------------------------------------------------

If you wish to install locally (i.e., without container), then:
